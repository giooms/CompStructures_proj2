...
loop_y:
    CMPLE(R6, R5, R7)     |; while (y <= yBottomRight)
    BF(R7, end_y)         

    MOVE(R1, R7)          |; x = xTopLeft
loop_x:
    CMPLE(R7, R4, R0)     |; while (x <= xBottomRight)
    BF(R0, end_x)

    |; Check if on border: if (x == xTopLeft || x == xBottomRight || y == yTopLeft || y == yBottomRight)
    CMPEQ(R7, R1, R0)     |; x == xTopLeft
    BT(R0, draw_pixel)
    CMPEQ(R7, R4, R0)     |; x == xBottomRight
    BT(R0, draw_pixel)
    CMPEQ(R6, R2, R0)     |; y == yTopLeft
    BT(R0, draw_pixel)
    CMPEQ(R6, R5, R0)     |; y == yBottomRight
    BF(R0, skip_pixel)
